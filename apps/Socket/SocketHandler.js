...
class SocketHandler {
    ...
    async broadcastOnlineStatus(userId, isOnline, lastSeen) {
        try {
            ...
            // Only hide status from blocked users (restrict is one-sided, doesn't affect status broadcasting)
            var hiddenFromUserIds = user.blockedUsers.map(function(id) { return id.toString(); });
    ...
    async sendOnlineUsersToUser(userId, socket) {
        try {
            ...
            // Restrict is one-sided: only hide users who BLOCKED me from my online view
            // (not users who restricted me — that's their private setting)
